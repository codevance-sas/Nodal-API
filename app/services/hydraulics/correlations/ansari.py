...
def calculate_ansari(data: HydraulicsInput) -> HydraulicsResult:
    ...
    for i in range(depth_steps-1):
        ...
        v_bs = 0.25 * np.sqrt(g * tubing_diameter * (liquid_density - gas_density) / liquid_density)
        
        # Slug-churn transition
        v_sc = 0.4 * np.sqrt(g * tubing_diameter)
        
        # Churn-annular transition
        v_ca = 3.5 * np.sqrt(g * tubing_diameter * (liquid_density - gas_density) / gas_density)
        ...
        if regime == "Bubble":
            ...
            v_tb = 0.35 * np.sqrt(g * tubing_diameter * (liquid_density - gas_density) / liquid_density)
            slug_fraction = 0.8
        ...
        dp_acceleration = 0.01 * dp_friction  # Simplified estimate
